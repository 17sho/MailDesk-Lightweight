...
class AddProxyDialog(QDialog):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        card_caption = QLabel("填写代理地址和可选的认证信息，保存后可用于全局或账号绑定。")
        card_caption.setObjectName("settingsCardCaption")
        card_caption.setWordWrap(True)
        card_layout.addWidget(card_title)
        card_layout.addWidget(card_caption)

        self.name_input = self._field(card_layout, "名称", "例如：香港节点 1")
        card_layout.addLayout(self._type_row())
        ...
        card_layout.addLayout(credentials)

        self.default_proxy = QCheckBox("设为默认代理（全局代理池会优先从这里开始）")
        card_layout.addWidget(self.default_proxy)
